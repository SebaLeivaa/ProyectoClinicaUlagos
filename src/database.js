...
//Funcion para validar formato del telefono
...
//Se configura la base de datos y se hace la conexion
...
app.post('/reservaUno',(req, res) => {
  ...
  const selectQuery = `
  SELECT 
    DATE(CM.CITA_DIS_FECHA) AS FECHA,
	  CM.CITA_DIS_FECHA,
    PR.PROF_NOMBRES, 
    PR.PROF_APELLIDOS, 
    PR.PROF_RUT, 
...
app.post('/anularHoraConsulta', (req, res) => {
  ...
  .then(resultados => {
    ...
    if(resultados.rows.length === 0){
      res.render('viewsPaciente/errorAnularHora', { error: 'Esta cuenta no existe', rutValido});
...
//Redirige a la pagina principal inicio sesion usuario
app.get('/inicioSesionUsuario', (req, res) => {
  res.render('viewsPaciente/inicioSesionUsuario');
});

//Redirige a la pagina especialidades (informacion) del lado del cliente 
app.get('/especialidades', (req, res) => {
  res.render('viewsPaginaPrincipal/especialidades');
});


//Redirige a la pagina profesionales (informacion) del lado del cliente 
app.get('/profesionales', (req, res) => {
  res.render('viewsPaginaPrincipal/profesionales');
});
...
//Se realiza la consulta de las horas del profesional, se muestran los datos de sus horas futuras
...
app.post('/guardarCambiosCitaFinal', (req, res) => {
  const { fechaCita, horaCita, salaCita, citaMedicaId } = req.body;
  const user = req.session.user;
  const rut = user.prof_rut;
  const fechaFinal = `${fechaCita} ${horaCita}`;
  ...
  Promise.all([
    pool.query(updateQuery, [salaCita, fechaFinal, citaMedicaId]),
    pool.query(selectQuery, [rut]),
    pool.query(selectQuery2, [rut]),
    pool.query(selectQuery3)
  ])
...
app.post('/detallesHistorialProfesional', (req, res) => {
  const user = req.session.user;
  const rut = user.prof_rut;

  const selectQuery = `
    SELECT PA.PACI_NOMBRE, PA.PACI_APELLIDO, PA.PACI_RUT, CM.CITA_DETALLE, ES.ESPE_NOMBRE, CM.CITA_SALA_NUM, CM.CITA_DIS_FECHA, CM.CITA_ID
	  FROM PACIENTE PA RIGHT JOIN CITA_MEDICA CM
	ON PA.PACI_RUT = CM.CITA_PACI_RUT 
		JOIN PROFESIONAL PR 
		ON CM.CITA_PROF_RUT = PR.PROF_RUT
  ...
  Promise.all([
    ...
    .then(([resultados, resultados2]) => {
      res.render('viewsProfesional/detallesHistorialProf', { resultados, resultados2} );
    })
    .catch(error => {
      console.error(error);
      res.status(500).send('Error interno del servidor');
...
app.post('/guardarDetalleCita', (req, res) => {
  const { citaMedicaId, detalleCita} = req.body;
  const user = req.session.user;
  const rut = user.prof_rut;
  ...
  pool.query(updateQuery, [detalleCita, citaMedicaId])
  .then( updateResultados => {
    console.log('Se ha actualizado correctamente el detalle de la cita medica');
    const selectQuery = `
    SELECT PA.PACI_NOMBRE, PA.PACI_APELLIDO, CM.CITA_DETALLE, PA.PACI_RUT, ES.ESPE_NOMBRE, CM.CITA_SALA_NUM, CM.CITA_DIS_FECHA, CM.CITA_ID
	    FROM PACIENTE PA RIGHT JOIN CITA_MEDICA CM
	    ON PA.PACI_RUT = CM.CITA_PACI_RUT 
		  JOIN PROFESIONAL PR 
		  ON CM.CITA_PROF_RUT = PR.PROF_RUT
...
app.post('/editarDetalleCita', (req, res) => {
  const { citaMedicaId, detalleCita} = req.body;
  const user = req.session.user;
  const rut = user.prof_rut;
